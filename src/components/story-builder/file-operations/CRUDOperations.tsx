...
const StateFormDialog: React.FC<StateFormProps> = ({ stateForm, loading, onSubmit, onChange, onCancel }) => (
  <DialogContent>
    <DialogHeader>
    ...
    <div className='grid gap-4 py-4'>
      ...
      <div className='grid grid-cols-4 items-center gap-4'>
        ...
        <Select value={stateForm.visibility} onValueChange={(value) => onChange('visibility', value as Visibility)}>
...
const SessionFormDialog: React.FC<StateFormProps> = ({ stateForm, loading, onSubmit, onChange, onCancel }) => (
  <DialogContent>
    <DialogHeader>
      ...
      <div className='grid gap-4 py-4'>
        ...
        {item.tags && item.tags.length > 0 && (
          <div className='grid grid-cols-4 items-start gap-4'>
            ...
            <div className='col-span-3'>{item.tags.join(', ')}</div>
          </div>
        )}
      </div>
      <DialogFooter>
  ...
  return (
    <div className='space-y-6'>
      ...
      <Dialog open={!!selectedItem} onOpenChange={(open) => !open && setSelectedItem(null)}>
        <MetadataDialog item={selectedItem} onClose={() => setSelectedItem(null)} />
      </Dialog>
      ...
      {!loading && !error && activeView === 'session' && (
        <DataList title='Sessions' items={sessions} onItemClick={handleItemClick} />
      )}
      {!loading && !error && activeView === 'state' && (
        <DataList title='States' items={states} onItemClick={handleItemClick} />
      )}
    </div>
  );
}
