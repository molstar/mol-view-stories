...
export function PublishModal() {
  ...
  const filteredStories = useMemo(() => {
    ...
    return stories.filter((story: StoryItem) =>
    ...
    <Dialog open={publishModal.isOpen} onOpenChange={handleClose}>
      <DialogContent className='max-w-md' onOpenAutoFocus={(e) => e.preventDefault()}>
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className='w-full'>
          ...
          <TabsContent value='new' className='space-y-4 mt-4'>
            <div className='p-4 bg-blue-50 border border-blue-200 rounded-lg'>
              <div className='flex items-start gap-3'>
                ...
                <div>
                  <h4 className='font-medium text-blue-900'>Publish as New Story</h4>
                  <p className='text-sm text-blue-700 mt-1'>
                    This will create a new story with the title &quot;{story.metadata.title}&quot;. 
                    Your existing stories will remain unchanged.
                  </p>
                </div>
              </div>
            </div>
          ...
          <TabsContent value='overwrite' className='space-y-4 mt-4'>
            {!auth.isAuthenticated ? (
              ...
              <>
                ...
                <div className='space-y-2 max-h-48 overflow-y-auto'>
                  <Label>Select story to overwrite:</Label>
                        ...
                        >
                          <div className='flex items-start gap-3'>
                            ...
                            <div className='flex-1 min-w-0'>
                              <div className='font-medium text-gray-900 truncate'>{storyItem.title}</div>
                              {storyItem.description && (
                                <div className='text-sm text-gray-600 truncate mt-1'>{storyItem.description}</div>
                              )}
                              <div className='text-xs text-gray-500 mt-1'>
                                Updated {new Date(storyItem.updated_at).toLocaleDateString()}
                              </div>
                            </div>
                          </div>
                        </button>
