...
def check_user_session_limit(user_id, max_sessions):
    ...
    if current_count >= max_sessions:
        raise APIError(
            f'Session limit reached. You have {current_count} sessions (limit: {max_sessions}). Please delete some sessions before creating new ones.',
    ...
    logger.info(f"User {user_id} session count check passed: {current_count}/{max_sessions}")
    return True
...
def check_user_story_limit(user_id, max_stories):
    ...
    if current_count >= max_stories:
        raise APIError(
            f'Story limit reached. You have {current_count} stories (limit: {max_stories}). Please delete some stories before creating new ones.',
            status_code=429,  # Too Many Requests
            details={
                'current_count': current_count,
                'limit': max_stories,
    ...
    logger.info(f"User {user_id} story count check passed: {current_count}/{max_stories}")
    return True 