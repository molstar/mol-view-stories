...
from storage.client import handle_minio_error, minio_client, MINIO_BUCKET, list_minio_objects, ensure_bucket_exists
from storage.metadata import validate_metadata, validate_data_filename, update_metadata_timestamp
...
def _validate_save_inputs(data_type, data, metadata):
    ...
    if not data_filename:
        raise APIError(
            f"Filename is required for {data_type}",
            status_code=400
        )
    ...
    if data_type == "story":
        ...
        if filename and filename.endswith('.mvsx'):
            ...
            if isinstance(story_data, str):
                ...
                logger.warning(f"MVSX story_data is a dict, not a string. Keys: {list(story_data.keys())}")
                for k, v in story_data.items():
                    logger.warning(f"Key: {k}, type: {type(v)}")
                if 'title' in story_data:
                    logger.error("MVSX story_data contains a 'title' field! This is likely a frontend or API payload bug.")
                data_bytes = json.dumps(story_data).encode('utf-8')
            else:
                logger.error(f"MVSX story_data is not a string or dict. Type: {type(story_data)}")
                data_bytes = b''
...
def _process_objects_for_user(objects, data_type, user_id):
    ...
    logger.debug(f"Found {len(object_dirs)} potential {data_type} directories for user {user_id}")
...
def _load_metadata_from_directory(dir_path, data_type):
    ...
    try:
        ...
        if actual_type == data_type:
            ...
            logger.warning(f"Skipping {metadata_path}: wrong type (expected {data_type}, got {actual_type})")
...
def delete_all_user_data(user_id):
    ...
    return _create_deletion_summary(
        user_id, sessions_deleted, stories_deleted, len(deleted_objects),
...
def _create_deletion_summary(user_id, sessions_deleted, stories_deleted, total_objects_deleted, message):
...
def _delete_object_by_id(object_id, requesting_user_id, object_type):
    ...
    if not object_id:
        raise APIError(f'{object_type.title()} ID is required for deletion', status_code=400)
    
    if not requesting_user_id:
        raise APIError('User authentication required', status_code=401)
    
    logger.info(f"Delete {object_type} {object_id} requested by user: {requesting_user_id}")
    ...
    logger.info(f"Successfully deleted {object_type} {object_id} for user {requesting_user_id}")
...
def _update_object_by_id(object_id, requesting_user_id, update_data, object_type):
    ...
    if not object_id:
        raise APIError(f'{object_type.title()} ID is required for update', status_code=400)
    
    if not requesting_user_id:
        raise APIError('User authentication required', status_code=401)
    
    logger.info(f"Update {object_type} {object_id} requested by user: {requesting_user_id}")
    ...
    updated_metadata = _update_object_metadata(matching_object, update_data, object_type)
    ...
    logger.info(f"Successfully updated {object_type} {object_id} for user {requesting_user_id}")
...
def _save_updated_story_data(object_path, metadata, update_data, story_id):
    ...
    for obj in objects:
        if not obj['key'].endswith('metadata.json') and obj['key'].startswith(object_prefix):
