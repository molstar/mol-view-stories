        ...
        def decorated_function(*args, **kwargs):
            ...
            if content_length is not None:
                if content_length > max_size_bytes:
                    logger.warning(f"Request payload too large: {content_length} bytes (max: {max_size_bytes} bytes)")
                    return jsonify({
                        'error': True,
                        ...
                        'details': {
                            ...
                            'description': f'The request payload exceeds the maximum allowed size of {max_size_mb_config}MB',
                ...
                logger.warning("Request missing Content-Length header - rejecting to prevent bypass")
                return jsonify({
                    'error': True,
                    ...
                    'details': {
                        ...
                        'description': f'Content-Length header is required for uploads. Maximum allowed size: {max_size_mb_config}MB',
                        'max_size_mb': max_size_mb_config,
...
class SizeLimitedStream:
    ...
    def read(self, size=-1):
        ...
        if self.bytes_read > self.max_size:
            self.logger.warning(f"Stream size limit exceeded: {self.bytes_read} bytes (max: {self.max_size} bytes)")
    ...
    def readline(self, size=-1):
        ...
        if self.bytes_read > self.max_size:
            self.logger.warning(f"Stream size limit exceeded during readline: {self.bytes_read} bytes")
            raise RequestEntityTooLarge(
                f"Request entity too large: {self.bytes_read} bytes (max: {self.max_size} bytes)"
            )
    ...
    def readlines(self, hint=-1):
        ...
        if self.bytes_read > self.max_size:
            self.logger.warning(f"Stream size limit exceeded during readlines: {self.bytes_read} bytes")
            raise RequestEntityTooLarge(
                f"Request entity too large: {self.bytes_read} bytes (max: {self.max_size} bytes)"
            )
...
class SizeValidationMiddleware:
        ...
        if content_length:
            try:
                ...
                if content_length > self.max_size_bytes:
                    self.logger.warning(f"WSGI middleware rejected large request: {content_length} bytes")
                    # Return 413 response
    ...
    def _create_error_response(self, received_size=None):
        ...
        if received_size:
            response_data['details']['received_size_mb'] = round(received_size / (1024 * 1024), 2)
