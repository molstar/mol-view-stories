...
from schemas import StoryInput, BaseItemUpdate
from storage import (
    create_metadata,
    save_object,
...
def story_by_id(story_id):
    ...
    if request.method == 'GET':
        ...
        # Add public URI
        matching_story['public_uri'] = generate_public_uri('story', story_id)
        ...
        if request.method == 'PUT':
            # PUT: Update story
            # Note: We can't use the decorator here since it's inside a conditional
            # But we can add inline validation for the PUT method
            content_length = request.content_length
            if content_length is not None:
                max_size_mb = current_app.config.get('MAX_UPLOAD_SIZE_MB', 50)
                max_size_bytes = max_size_mb * 1024 * 1024
                if content_length > max_size_bytes:
                    logger.warning(f"PUT request payload too large: {content_length} bytes (max: {max_size_bytes} bytes)")
                    return jsonify({
                        'error': True,
            ...
            logger.info(f'Update story request received for story_id: {story_id} by user: {user_id}')
            logger.debug(f'Validated input fields: {validated_input.dict(exclude_none=True)}')
            
            # Perform the update with authorization check
            updated_metadata = update_story_by_id(story_id, user_id, validated_input.dict(exclude_none=True))
            ...
            # DELETE: Delete story
            logger.info(f'Delete story request received for story_id: {story_id} by user: {user_id}')
...
def get_story_data(story_id):
    ...
    try:
        ...
        if requested_format in ['mvsj', 'mvsx']:
            extensions_to_try = [f'.{requested_format}']
        else:
            extensions_to_try = ['.mvsj', '.mvsx']
        ...
        for ext in extensions_to_try:
            ...
            try:
                ...
                continue
        if requested_format:
            logger.error(f"No valid story data found for story: {story_id} with format: {requested_format}")
            raise APIError(f'Story data not found in {requested_format} format', status_code=404)
        else:
...
def get_story_format(story_id):
    ...
    try:
        ...
        raise APIError('Error determining story format', status_code=500, details={"error": str(e)}) 