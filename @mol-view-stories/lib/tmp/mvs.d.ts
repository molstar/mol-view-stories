 ...
 * @since 1.0.0
 */
declare class Type<A, O = A, I = unknown>
 ...
 * @since 1.0.0
 */
declare class RefinementType<C extends Any, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  readonly type: C;
 ...
 * @since 1.0.0
 */
declare class ArrayType<C extends Any, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  readonly type: C;
  /**
   * @since 1.0.0
 ...
 * @since 1.5.3
 */
interface ArrayC<C extends Mixed>
  extends ArrayType<C, Array<TypeOf<C>>, Array<OutputOf<C>>, unknown> {}
/**
 * @since 1.0.0
 */
declare class InterfaceType<P, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  readonly props: P;
  /**
   * @since 1.0.0
 ...
 * @since 1.5.3
 */
interface TypeC<P extends Props> extends
  InterfaceType<
    P,
    {
 ...
 * @since 1.0.0
 */
declare class PartialType<P, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  readonly props: P;
  /**
   * @since 1.0.0
 ...
 * @since 1.5.3
 */
interface PartialC<P extends Props> extends
  PartialType<
    P,
    {
      [K in keyof P]?: TypeOf<P[K]>;
 ...
 * @since 1.5.3
 */
interface RecordC<D extends Mixed, C extends Mixed> extends
  DictionaryType<
    D,
    C,
    {
 ...
 * @since 1.0.0
 */
declare class UnionType<CS extends Array<Any>, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  readonly types: CS;
  /**
   * @since 1.0.0
 ...
 * @since 1.0.0
 */
declare class IntersectionType<
 ...
 * @since 1.0.0
 */
declare class TupleType<CS extends Array<Any>, A = any, O = A, I = unknown>
  extends Type<A, O, I> {
  readonly types: CS;
  /**
   * @since 1.0.0
 ...
 * @since 1.5.3
 */
interface TupleC<CS extends [Mixed, ...Array<Mixed>]> extends
  TupleType<
    CS,
    CS extends {
      length: 1;
 ...
 * @since 1.5.3
 */
interface RefinementC<C extends Any, B = TypeOf<C>>
  extends RefinementType<C, B, OutputOf<C>, InputOf<C>> {}
...
type AllowedValueTypes =
  ...
  | {};
interface FieldBase<
  V extends AllowedValueTypes = any,
...
interface RequiredField<V extends AllowedValueTypes = any>
  extends FieldBase<V> {
  required: true;
}
declare function RequiredField<V extends AllowedValueTypes>(
  type: Type<V>,
  description: string,
): RequiredField<V>;
/** Schema for param field which can be dropped (meaning that a default value will be used) */
interface OptionalField<V extends AllowedValueTypes = any>
  extends FieldBase<V> {
  required: false;
...
declare function OptionalField<V extends AllowedValueTypes>(
  type: Type<V>,
  defaultValue: DefaultValue<V>,
  description: string,
): OptionalField<V>;
/** Schema for one field in params (i.e. a value in a top-level key-value pair) */
type Field<V extends AllowedValueTypes = any> =
  | RequiredField<V>
  | OptionalField<V>;
...
type ValueFor<F extends Field | Any> = F extends Field<infer V> ? V
  : F extends Any ? TypeOf<F>
  : never;
...
declare function SimpleParamsSchema<TFields extends Fields>(
  fields: TFields,
): SimpleParamsSchema<TFields>;
type ValuesForFields<F extends Fields> =
  & {
    [key in keyof F as (F[key] extends RequiredField<any> ? key : never)]:
      ValueFor<F[key]>;
  }
  & {
    [key in keyof F as (F[key] extends OptionalField<any> ? key : never)]?:
      ValueFor<F[key]>;
  };
type ValuesForSimpleParamsSchema<TSchema extends SimpleParamsSchema> =
  ValuesForFields<TSchema['fields']>;
type Cases = {
...
interface UnionParamsSchema<
  TDiscriminator extends string = string,
  TCases extends Cases = Cases,
> {
  type: 'union';
...
declare function UnionParamsSchema<
  TDiscriminator extends string,
  TCases extends Cases,
>(
  discriminator: TDiscriminator,
  discriminatorDescription: string,
  cases: TCases,
...
type ValuesFor<P extends ParamsSchema> = P extends SimpleParamsSchema
  ? ValuesForSimpleParamsSchema<P>
...
type Node<TKind extends string = string, TParams extends {} = {}> = {} extends
  TParams ? {
    kind: TKind;
    params?: TParams;
    custom?: CustomProps;
...
type Tree<
  TNode extends Node<string, {}> = Node<string, {}>,
...
type SubtreeOfKind<
  TTree extends Tree,
  TKind extends Kind<Subtree<TTree>> = Kind<Subtree<TTree>>,
> = RootOfKind<Subtree<TTree>, TKind>;
type RootOfKind<TTree extends Tree, TKind extends Kind<TTree>> = Extract<
  TTree,
  Tree<any, Node<TKind>>
>;
...
type ParamsOfKind<
  TTree extends Tree,
  TKind extends Kind<Subtree<TTree>> = Kind<Subtree<TTree>>,
> = NonNullable<SubtreeOfKind<TTree, TKind>['params']>;
type ParamsSchemas = {
...
interface TreeSchema<
  TParamsSchemas extends ParamsSchemas = ParamsSchemas,
  TRootKind extends keyof TParamsSchemas = string,
> {
...
declare function TreeSchema<
  P extends ParamsSchemas = ParamsSchemas,
  R extends keyof P = string,
>(schema: TreeSchema<P, R>): TreeSchema<P, R>;
/** ParamsSchemas per node kind */
type ParamsSchemasOf<TTreeSchema extends TreeSchema> = TTreeSchema extends
  TreeSchema<infer TParamsSchema, any> ? TParamsSchema : never;
/** Type of tree node which can occur as the root of a tree conforming to tree schema `TTreeSchema` */
type RootFor<TTreeSchema extends TreeSchema> = NodeFor<
  TTreeSchema,
  TTreeSchema['rootKind']
>;
...
type NodeFor<
  TTreeSchema extends TreeSchema,
  TKind extends keyof ParamsSchemasOf<TTreeSchema> = keyof ParamsSchemasOf<
    TTreeSchema
  >,
> = {
  [key in keyof ParamsSchemasOf<TTreeSchema>]: Node<
    key & string,
    ValuesFor<ParamsSchemasOf<TTreeSchema>[key]>
  >;
}[TKind];
/** Type of tree which conforms to tree schema `TTreeSchema` */
type TreeFor<TTreeSchema extends TreeSchema> = Tree<
  NodeFor<TTreeSchema>,
  RootFor<TTreeSchema> & NodeFor<TTreeSchema>
>;
...
type ComponentSelectorT =
  ...
  | 'water'
  | 'coarse';
declare const ComponentSelectorT: Type<
  ComponentSelectorT,
  ComponentSelectorT,
  unknown
>;
...
type LabelAttachments =
  ...
  | 'top-center'
  | 'top-right';
declare const LabelAttachments: Type<
  LabelAttachments,
  LabelAttachments,
  unknown
>;
...
declare const _PrimitiveComponentExpressionT: PartialC<{
  ...
  expressions: ArrayC<
    Type<ComponentExpressionT, ComponentExpressionT, unknown>
  >;
}>;
/** Primitives-related types */
interface PrimitiveComponentExpressionT
  extends ValueFor<typeof _PrimitiveComponentExpressionT> {
}
declare const PrimitiveComponentExpressionT: Type<
  PrimitiveComponentExpressionT
>;
declare const PrimitivePositionT: UnionC<
  [
    Type<Vector3, Vector3, unknown>,
    Type<ComponentExpressionT, ComponentExpressionT, unknown>,
    Type<PrimitiveComponentExpressionT, PrimitiveComponentExpressionT, unknown>,
  ]
>;
...
declare const _CategoricalPalette: IntersectionC<[
  TypeC<{
  ...
  PartialC<{
    colors: UnionC<
      [
        Type<ColorListNameT, ColorListNameT, unknown>,
        Type<ColorDictNameT, ColorDictNameT, unknown>,
        ArrayC<Type<ColorT, ColorT, unknown>>,
        RecordC<StringC, Type<ColorT, ColorT, unknown>>,
      ]
    >;
...
declare const _DiscretePalette: IntersectionC<[
  TypeC<{
  ...
  PartialC<{
    ...
    colors: UnionC<
      [
        Type<ColorListNameT, ColorListNameT, unknown>,
        ArrayC<Type<ColorT, ColorT, unknown>>,
        ArrayC<TupleC<[Type<ColorT, ColorT, unknown>, NumberC]>>,
        ArrayC<
          TupleC<
            [
              Type<ColorT | null, ColorT | null, unknown>,
              Type<number | null, number | null, unknown>,
              Type<number | null, number | null, unknown>,
            ]
          >
        >,
...
declare const _ContinuousPalette: IntersectionC<[
  TypeC<{
...
type MVSAnimationNodeParams<TKind extends MVSAnimationKind> = ParamsOfKind<
  MVSAnimationTree,
  TKind
>;
type MVSAnimationSubtree<TKind extends MVSAnimationKind = MVSAnimationKind> =
  SubtreeOfKind<MVSAnimationTree, TKind>;
...
declare const MVSTreeSchema: TreeSchema<{
  ...
  component: SimpleParamsSchema<{
    ...
    selector: RequiredField<
      ComponentSelectorT | ComponentExpressionT | ComponentExpressionT[]
    >;
  }>;
  ...
  color_from_uri: SimpleParamsSchema<{
    ...
    palette: OptionalField<
      CategoricalPalette | DiscretePalette | ContinuousPalette | null
    >;
    /** URL of the annotation resource. */
    uri: RequiredField<string>;
    /** Format of the annotation resource. */
  ...
  primitive: UnionParamsSchema<'kind', {
    ...
    tube: SimpleParamsSchema<{
      tooltip: OptionalField<string | null>;
      start: RequiredField<
        ComponentExpressionT | Vector3 | PrimitiveComponentExpressionT
      >;
      end: RequiredField<
        ComponentExpressionT | Vector3 | PrimitiveComponentExpressionT
      >;
    ...
    arrow: SimpleParamsSchema<{
      ...
      >;
        ...
        ComponentExpressionT | Vector3 | PrimitiveComponentExpressionT | null
      >;
    ...
    ellipse: SimpleParamsSchema<{
      ...
      center: RequiredField<
        ComponentExpressionT | Vector3 | PrimitiveComponentExpressionT
      >;
      major_axis: OptionalField<Vector3 | null>;
      minor_axis: OptionalField<Vector3 | null>;
      major_axis_endpoint: OptionalField<
        ComponentExpressionT | Vector3 | PrimitiveComponentExpressionT | null
      >;
      minor_axis_endpoint: OptionalField<
        ComponentExpressionT | Vector3 | PrimitiveComponentExpressionT | null
      >;
...
type MVSSubtree<TKind extends MVSKind = MVSKind> = SubtreeOfKind<
  MVSTree,
  TKind
>;
...
declare class _Base<TKind extends MVSKind> {
  ...
  ): MVSSubtree<TChildKind>;
}
/** MVS builder pointing to the 'root' node */
declare class Root extends _Base<'root'>
  implements FocusMixin, PrimitivesMixin {
  ...
  primitives: (
    params?: MVSNodeParams<'primitives'> & CustomAndRef,
  ) => Primitives;
  primitivesFromUri: (
    params: MVSNodeParams<'primitives_from_uri'> & CustomAndRef,
  ) => PrimitivesFromUri;
  animation(
    params?: MVSAnimationNodeParams<'animation'> & CustomAndRef,
  ): Animation;
...
declare class Animation {
  ...
  interpolate(
    params: MVSAnimationNodeParams<'interpolate'> & CustomAndRef,
  ): Animation;
}
...
declare class Parse extends _Base<'parse'> {
  ...
  modelStructure(
    params?:
      & Pick<
        MVSNodeParams<'structure'>,
        typeof StructureParamsSubsets['model'][number]
      >
      & CustomAndRef,
  ): Structure;
  ...
  assemblyStructure(
    params?:
      & Pick<
        MVSNodeParams<'structure'>,
        typeof StructureParamsSubsets['assembly'][number]
      >
      & CustomAndRef,
  ): Structure;
  ...
  symmetryStructure(
    params?:
      & Pick<
        MVSNodeParams<'structure'>,
        typeof StructureParamsSubsets['symmetry'][number]
      >
      & CustomAndRef,
  ): Structure;
  ...
  symmetryMatesStructure(
    params?:
      & Pick<
        MVSNodeParams<'structure'>,
        typeof StructureParamsSubsets['symmetry_mates'][number]
      >
      & CustomAndRef,
  ): Structure;
...
declare class Structure extends _Base<'structure'>
  implements PrimitivesMixin, TransformMixin {
  ...
  component(
    params?: Partial<MVSNodeParams<'component'>> & CustomAndRef,
  ): Component;
  ...
  componentFromUri(
    params: MVSNodeParams<'component_from_uri'> & CustomAndRef,
  ): Component;
  ...
  componentFromSource(
    params: MVSNodeParams<'component_from_source'> & CustomAndRef,
  ): Component;
  ...
  labelFromUri(
    params: MVSNodeParams<'label_from_uri'> & CustomAndRef,
  ): Structure;
  ...
  labelFromSource(
    params: MVSNodeParams<'label_from_source'> & CustomAndRef,
  ): Structure;
  ...
  tooltipFromUri(
    params: MVSNodeParams<'tooltip_from_uri'> & CustomAndRef,
  ): Structure;
  ...
  tooltipFromSource(
    params: MVSNodeParams<'tooltip_from_source' & CustomAndRef>,
  ): Structure;
  transform: (
    params?:
      | ({} & {
...
declare class Component
  ...
  representation(
    params?: Partial<MVSNodeParams<'representation'>> & CustomAndRef,
  ): Representation;
...
declare class Representation extends _Base<'representation'> {
  ...
  colorFromUri(
    params: MVSNodeParams<'color_from_uri'> & CustomAndRef,
  ): Representation;
  ...
  colorFromSource(
    params: MVSNodeParams<'color_from_source'> & CustomAndRef,
  ): Representation;
...
/** MVS builder pointing to a 'component' or 'component_from_uri' or 'component_from_source' node */
declare class Volume extends _Base<'volume'>
  implements FocusMixin, TransformMixin {
  /** Add a 'representation' node and return builder pointing to it. 'representation' node instructs to create a visual representation of a component. */
  representation(
    params?: MVSNodeParams<'volume_representation'> & CustomAndRef,
...
declare class VolumeRepresentation extends _Base<'volume_representation'>
  implements FocusMixin {
  /** Add a 'color' node and return builder pointing back to the representation node. 'color' node instructs to apply color to a visual representation. */
  color(params: MVSNodeParams<'color'> & CustomAndRef): VolumeRepresentation;
  /** Add an 'opacity' node and return builder pointing back to the representation node. 'opacity' node instructs to customize opacity/transparency of a visual representation. */
  opacity(
    params: MVSNodeParams<'opacity'> & CustomAndRef,
  ): VolumeRepresentation;
  focus: (
    params?:
      | ({} & {
...
type MVSPrimitiveSubparams<TKind extends MVSNodeParams<'primitive'>['kind']> =
  Omit<
    Extract<MVSNodeParams<'primitive'>, {
      kind: TKind;
    }>,
    'kind'
  >;
...
declare class Primitives extends _Base<'primitives'> implements FocusMixin {
  ...
  distance(
    params: MVSPrimitiveSubparams<'distance_measurement'> & CustomAndRef,
  ): Primitives;
  /** Defines an angle between vectors (b - a) and (c - b). */
  angle(
    params: MVSPrimitiveSubparams<'angle_measurement'> & CustomAndRef,
  ): Primitives;
  /** Defines a label. */
  ...
  ellipsoid(
    params: MVSPrimitiveSubparams<'ellipsoid'> & CustomAndRef,
  ): Primitives;
...
declare class PrimitivesFromUri extends _Base<'primitives_from_uri'>
  implements FocusMixin {
  focus: (
    params?:
      | ({} & {
...
declare namespace Vec3 {
   ...
   */
  function transformDirectionOffset(
    out: NumberArray,
    a: NumberArray,
    m: NumberArray,
    outO: number,
   ...
   * @param radius [0, +Inf]
   */
  function directionFromSpherical(
    out: Vec3,
    inclination: number,
    azimuth: number,
    radius: number,
