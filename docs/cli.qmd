# MolViewStories CLI Tool

::: {.callout-warning}
## Development Status
The CLI is currently in active development. Features and commands may change.
:::

In other sections we have described that a Story is a collection of scenes where each scene is defined by having
a molecular description in MolViewSpec (and created using Javascript) and a narrative component written in markdown. Stories
also have shared metadata.  The structure that we have built up to create stories on the web can also be reused locally by recreating
the Story in the form of a nested set of files. In this case a story would consist of a file tree that looks something like the
files structure below.

The MVS CLI is a way to reuse the `StoryManager` class of the `mol-view-stories` repository and allow us to build stories form
the command line.
...
The best way to get started is to build a preexisting story from our repo. We will work on creating a quarto template
that will help users get started on new stories or incorporating them into existing website workflows.
