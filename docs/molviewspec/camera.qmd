...
- **`target`**: Look-at point `[x, y, z]` (where the camera looks)
- **`position`**: Camera position `[x, y, z]` (where the camera is)
- **`up`** (optional): Up direction `[x, y, z]` (default: `[0, 1, 0]`)
...
![Camera parameters](https://molstar.org/mol-view-spec/docs/files/camera.png)
...
- **`background_color`**: Color value (named color or hex)
...
```{.molviewspec}
builder.canvas({ background_color: '#2c3e50' });
