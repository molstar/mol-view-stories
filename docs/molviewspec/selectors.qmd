...
// Try changing 'polymer' to 'ligand', 'water', or 'protein'
structure
...
### Example: Selecting Specific Atoms
...
The `instance_id` field distinguishes different instances of the same chain created by symmetry operators (crystals) or assembly operations.

### Crystals

Crystal symmetry instances use IDs in the form `n_klm` following [mmCIF conventions](https://mmcif.wwpdb.org/dictionaries/mmcif_pdbx_v50.dic/Items/_struct_conn.ptnr1_symmetry.html):

- Standard: `1_555`, `2_454` (indices 0-9)
- Extended: `1_(11)15`, `1_1(11)5`, `1_(-1)1(-1)` (indices outside 0-9 use parentheses)

### Assemblies

Assembly instances use IDs based on [wwPDB naming conventions](https://www.wwpdb.org/news/news?year=2022#62559153c8eabd0c4864f208):

- Single operator: `ASM-1`, `ASM-2`, `ASM-3`
- Multiple operators: `ASM-X0-1`, `ASM-1-61` (dash-separated, rightmost applied first)

Example:
```javascript
// Select specific assembly instance
{ label_asym_id: 'A', instance_id: 'ASM-1' }
```

## Using Refs for Multi-Structure Selections

When working with multiple structures, you can set a `ref` (anchor) on nodes and reference it in selectors:

```javascript
// Set a ref on the structure
const structure = builder
    .download({ url: '...' })
    .parse({ format: 'mmcif' })
    .modelStructure({ ref: 'protein1' });

// Reference it in primitives or other contexts
builder
    .primitive({
        structure_ref: 'protein1',
        selector: { auth_seq_id: 508 }
    });
```

## Advanced: Color-Specific Selectors

The `color()` node also accepts selectors to apply different colors to different parts of a representation:
...
For complex coloring or selection schemes, consider using [Annotations](annotations.qmd) instead. Annotations allow you to define selections in external files (JSON, CIF, or BCIF format) and reference them in your MVS tree.

## See Also

- [Annotations](annotations.qmd) - External selection files
- [Tree Schema](tree-schema.qmd) - Complete node reference
- [Component Node Reference](tree-schema.qmd#component) - Detailed component parameters
